...
class RScale:
    def __init__(self, prob_scale=0.5, prob_rotate=0.5):
        ...
        self.scale = ShiftScaleRotate(rotate_limit=45,
                                      shift_limit=0.1,
                                      scale_limit=0.1,
                                      p = 1)
    ...
    def apply(self, img, mask):
        ...
        if random.random() < self.prob_scale:
            ...
            mask = self.scale.apply_to_mask(mask, angle = angles, scale = rand_scale, dx=0, dy=0)
        ...
        if random.random() < self.prob_rotate:
            ...
            mask = self.scale.apply_to_mask(mask, angle = rand_angle, scale = scales, dx=0, dy=0)
...
class RandomBrightnessContrast:
    def __init__(self, limit_alpha=.4, limit_beta = 0.4, prob=.1):
        self.limit_alpha = limit_alpha
        self.limit_beta = limit_beta
        self.prob = prob

    def apply(self, img, mask, alpha=1., beta=0.):
        if random.random() < self.prob:
