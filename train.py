...
def split_train_valid(fold_to_train = 0):
  ...
  for i in tqdm_notebook(range(train_df.ImageId.nunique())):
    ...
    labels.append( train_df[train_df.ImageId == im_name]['hasMask'].values.astype(int) )
...
train_dataset = Dataset('train_images/',train_df.iloc[trn_indexes],
                        transform=train_transforms, t_mode = 'train')
valid_dataset = Dataset('train_images/', train_df.iloc[val_indexes], transform=test_transforms, t_mode = 'valid')
...
scheduler_warmup = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=np.sqrt(10))
...
for epoch in range(start_epoch, NUM_EPOCHS):
    ...
    for image, masks in tqdm_notebook(data.DataLoader(train_dataset, batch_size = BATCH_SIZE, shuffle = True,
                                                num_workers=4, worker_init_fn=_worker_init_fn), disable=True):
    ...
    for image, masks in tqdm_notebook(data.DataLoader(valid_dataset, batch_size = BATCH_SIZE, shuffle = False,
                                                num_workers=4, worker_init_fn=_worker_init_fn), disable=True):
        ...
        for s in range(y_pred.shape[0]):
          ...
          for channel in range(4):
            ...
            if ((y_pred[s,channel,:,:].cpu().data.numpy().astype(int)).sum() < 512):
              
              sample_score += dice( masks[s,channel,:,:].cpu().data.numpy().astype(int),
                                    np.zeros((320,480)) ) / 4
              
            else:
              
              sample_score += dice( y_pred[s,channel,:,:].cpu().data.numpy().astype(int),
                                    masks[s,channel,:,:].cpu().data.numpy().astype(int) ) / 4
    ...
    if unet_score > best_score:
      save_checkpoint("{}_fold{}.pth".format(enc_name, Global_fold, epoch), model, optimizer)
